...
fn main() {
    ...
    for _ in 0..M{
        ...
        let c: usize = C.iter().map(|&x| 1<<x).sum();
        ac.push((a,c));
    }
    let mut queue = BinaryHeap::new();
    queue.push((Reverse(0), 0));
    let mut cost = vec![std::usize::MAX; 1 << N];
    while let Some((Reverse(x), v)) = queue.pop(){
        if cost[v] < std::usize::MAX{continue;}
        cost[v] = x;
        for &(a,c) in &ac{
            if cost[v] < std::usize::MAX{
                queue.push((Reverse(x+a), v|c));
            }
        }
    }
    if cost[(1<<N) -1] == std::usize::MAX{
        println!("-1")
    }else{
        println!("{}", cost[(1<<N)-1])
    }
