...
fn main() {
    ...
    let base_y = 1000_000;
    let mut tree = UnionFind::new(base_y*2);
    for &(x,y) in &XY{
        tree.union(x, y+base_y);
    }
    let mut X = vec![HashSet::new(); base_y*2];
    let mut Y = vec![HashSet::new(); base_y*2];
    for &(x,y) in &XY{
        X[tree.find(x)].insert(x);
        Y[tree.find(y+base_y)].insert(y);
    }
    let mut ans = 0;
    for i in 0..base_y*2{
        ans += X[i].len()*Y[i].len();
