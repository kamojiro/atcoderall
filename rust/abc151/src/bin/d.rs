...
fn main() {
    ...
    for i in 0..H{
        for j in 0..W{
            ans = ans.max(bfs((i,j), H, W, &S))
        }
    }
    println!("{}", ans);
}

fn bfs(s: (usize, usize), H: usize, W: usize, S: &Vec<Vec<u8>>) -> i32{
    if S[s.0][s.1] == b'#'{return 0}
    ...
    *(0..H).map(|i| board[i].iter().max().unwrap()).max().unwrap()
}
