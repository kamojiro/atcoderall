...
fn main() {
    ...
    for i in 0..(N+1-K){
        if A[i] >= 0{
            ans += ModInt::new(A[i] as usize)*factorial[N-1-i]*inversed_factorial[K-1]*inversed_factorial[N-K-i]
        }else{
            ans += (ModInt::new(0) - ModInt::new((- A[i]) as usize))*factorial[N-1-i]*inversed_factorial[K-1]*inversed_factorial[N-K-i]
        }
    }
    A.sort();
    for i in 0..(N+1-K){
