...
fn main() {
    ...
    for i in 0..(M-2){
        ...
        for j in (i+1)..(M-1){
            ...
            for k in (j+1)..M{
                let x = q[k];
                let a:u64 = max(Z[v].0, max(Z[w].0, Z[x].0));
                let b:u64 = max(Z[v].1, max(Z[w].1, Z[x].1));
                let c:u64 = max(Z[v].2, max(Z[w].2, Z[x].2));
                let d:u64 = max(Z[v].3, max(Z[w].3, Z[x].3));
                let e:u64 = max(Z[v].4, max(Z[w].4, Z[x].4));
                // let t = vec![a,b,c,d,e].iter().min().unwrap();
                ans = max(ans, *vec![a,b,c,d,e].iter().min().unwrap());
                // if t > &ans{
                //     ans = *t;
                // }
            }
