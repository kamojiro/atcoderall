...
fn main() {
    ...
    let mut Cost = vec![vec![std::u64::MAX; C]; R];
    ...
    while let Some((Reverse(cost), x, y)) = q.pop(){
        ...
        if y < C-1{
            if !visited[x][y+1]{
                if cost + A[x][y] < Cost[x][y+1]{
                    Cost[x][y+1] = cost + A[x][y];
                    q.push((Reverse(Cost[x][y+1]), x, y+1));
                }
            }
        }
        if y > 0{
            if !visited[x][y-1]{
                if cost + A[x][y-1] < Cost[x][y-1]{
                    Cost[x][y-1] = cost + A[x][y-1];
                    q.push((Reverse(Cost[x][y-1]), x, y-1));
                }
            }
        }
        if x < R-1{
            if !visited[x+1][y]{
                if cost + B[x][y] < Cost[x+1][y]{
                    Cost[x+1][y] = cost + B[x][y];
                    q.push((Reverse(Cost[x+1][y]), x+1, y));
                }
            }
        }
        for i in 1..(x+1 as usize){
            if !visited[x-i][y]{
                if cost + (1+i as u64) < Cost[x-i][y]{
                    Cost[x-i][y] = cost +  (1+i as u64);
                    q.push((Reverse(Cost[x-i][y]), x-i, y));
                }
            }
        }
    }

    println!("{}", Cost[R-1][C-1]);
