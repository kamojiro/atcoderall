...
fn main() {
    ...
    let T = 100;
    let mut ans = minh(K,T,-1000.0, 1000.0, &XYC);
    // for i in 0..10{
    //     let p = ((i as f64) - 5.0)*200.0;
    //     ans = ans.min(minh(K,T,p, p+100.0, &XYC));
    // }
    let X = (0..N).map(|i| XYC[i].0).sorted_by(|a,b| a.partial_cmp(b).unwrap()).collect_vec();
    for i in 0..N/3{
        let p = X[i*3];
        let q = X[i*3+1];
        ans = ans.min(minh(K,T,p, q, &XYC));
    }
    // for i in 0..20{
    //     let p = ((i as f64) - 10.0)*100.0;
    //     ans = ans.min(minh(K,T,p, p+100.0, &XYC));
    // }
    ...
    ans = ans.min(minh(K,T,-1000.0, 1000.0, &YXC));
    for i in 0..10{
        let p = ((i as f64) - 5.0)*200.0;
        ans = ans.min(minh(K,T,p, p+100.0, &YXC));
    }
    let Y = (0..N).map(|i| XYC[i].1).sorted_by(|a,b| a.partial_cmp(b).unwrap()).collect_vec();
    for i in 0..N/3{
        let p = Y[i*3];
        let q = Y[i*3+1];
        ans = ans.min(minh(K,T,p, q, &YXC));
    }

    println!("{}", ans)
}

fn minh(K: usize, T: usize, nlx: f64, nrx: f64, XYC: &Vec<(f64,f64,f64)>) -> f64{
    let mut lx = nlx;
    let mut lxt = miny(K, T, lx, &XYC);
    let mut rx = nrx;
    let mut rxt = miny(K, T, rx, &XYC);
    for _ in 0..T{
        let x1 = (lx*2.0+rx)/3.0;
        let x2 = (lx+rx*2.0)/3.0;
        let x1t = miny(K,T,x1,&XYC);
        let x2t = miny(K,T,x2,&XYC);
        if x1t < x2t{
            rx = x2;
            rxt = x2t;
        }else{
...
fn miny(K: usize,T: usize,x: f64, XYC: &Vec<(f64,f64,f64)>) -> f64{
    let mut ly = -1000.0;
    let mut lyt = get_time(K, x, ly, XYC);
    let mut ry = 1000.0;
    let mut ryt = get_time(K, x, ry, XYC);
    for _ in 0..T{
        let y1 = (ly*2.0+ry)/3.0;
        let y2 = (ly+ry*2.0)/3.0;
        let y1t = get_time(K, x, y1, XYC);
        let y2t = get_time(K, x, y2, XYC);
        if y1t < y2t{
            ry = y2;
            ryt = y2t;
        }else{
...
fn get_time(K: usize, x:f64, y:f64, XYC: &Vec<(f64,f64,f64)>) -> f64{
    let mut meat = Vec::new();
    for &(tx,ty,c) in XYC{
        meat.push(c*(((tx-x)*(tx-x) + (ty-y)*(ty-y)).sqrt()))
    }
    meat.sort_by(|a,b| a.partial_cmp(b).unwrap());
    meat[K-1]
}
