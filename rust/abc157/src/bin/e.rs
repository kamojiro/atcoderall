...
fn main() {
    ...
    let mut segs = vec![SegmentTree::new(N, || 0, |a,b| a+b);26];
    for (i,&s) in S.iter().enumerate(){
        let t = (s - b'a') as usize;
        segs[t].update(i, 1)
    }
    ...
    for _ in 0..Q{
        ...
        if t == 1{
            ...
            let s = c as u8;
            if S[i] == s{continue;}
            segs[(S[i] - b'a') as usize].update(i, 0);
            segs[(s - b'a') as usize].update(i, 1);
            S[i] = s;
        }
        if t == 2{
            input!{
