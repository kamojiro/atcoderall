...
fn main() {
    ...
    for i in 0..N{
        ...
        for j in 0..N{
            if i == j{continue;}
            args.push(Complex::<f64>::new(XY[j].0 - XY[i].0, XY[j].1 - XY[i].1).arg().to_degrees());
        }
        args.sort_by(|a, b| a.partial_cmp(b).unwrap());
        // debug_eprintln!("{:?}", args);
        for arg in &args{
            let mut reverse_arg = arg + 180.0;
            if reverse_arg > 180.0{
                reverse_arg -= 360.0;
            }
            let index = lower_bound(&args, reverse_arg);
            let front_index = index%(N-1);
            let back_index = (index + (N-2))%(N-1);
            let candidates = vec![(arg - args[front_index]).abs(), (arg - args[back_index]).abs()];
            // debug_eprintln!("{:?}", candidates);
            for another_arg in &candidates{
                let mut checked = *another_arg;
                while checked > 180.0{
                    checked = 360.0 - checked;
                }
                if ans < checked{
                    ans = checked;
                }
...
fn lower_bound(A: &Vec<f64>, a: f64) -> usize {
    let N = A.len();
    if A[N-1] < a{return N}
    let mut l = 0;
    let mut r = N;
