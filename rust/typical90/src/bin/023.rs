...
fn main() {
    ...
    let non_one_continuous = make_non_one_continuous(W);
    // for i in 0..=W{
    //     for j in 0..non_one_continuous[i].len(){
    //         print!("{:b}  ", non_one_continuous[i][j])
    //     }
    //     println!("");
    // }
    // eprintln!("{:?}", non_one_continuous);
    // eprintln!("{}", non_one_continuous[W].len());
    let mut state = vec![vec![]; W];
    for j in 0..non_one_continuous[W].len(){
        state[0].push(non_one_continuous[W][j]<<1);
        let t = (non_one_continuous[W][j]<<1) + 1;
        if pre_judge(W, 0, t){
            state[0].push((non_one_continuous[W][j]<<1) + 1);
        }
    }
    // if H == 1 || W == 1{
    //     let k = H.max(W);
    ...
    for i in 1..W{
        for j in 0..non_one_continuous[i].len(){
            for k in 0..non_one_continuous[W+1-i].len(){
                // eprintln!("{:b} {:b}", non_one_continuous[i][j], non_one_continuous[W+1-i][k]);
                let t = (non_one_continuous[i][j]<<(W+1-i)) + non_one_continuous[W+1-i][k];
                // eprintln!("{} {:b}", i, t);
                if pre_judge(W, i, t){
                    state[i].push(t)
                }
    ...
    // make transition
    let mut numbering = vec![HashMap::new(); W];
    for w in 0..W{
        for (i, x) in state[w].iter().enumerate(){
            numbering[w].insert(x, i);
        }
    }
    let mut transition: Vec<Vec<Vec<usize>>> = Vec::new();
    let dummy: usize = (1<<(W+1))-1;
    for w in 0..W{
        let nextw = (w+1)%W;
        let mut transition_w: Vec<Vec<usize>> = Vec::new();
        for i in 0..state[w].len(){
            let t = state[w][i];
            let t0 = t >> 1;
            let t1 = t0 + (1 << W);
            let mut transision_w_i: Vec<usize> = Vec::new();
            if let Some(&index) = numbering[nextw].get(&t0){
                transision_w_i.push(index)
            }else{
                transision_w_i.push(dummy)
            }
            if judge(W, w, t){
                if let Some(&index) = numbering[nextw].get(&t1){
                    // eprintln!("{} {:b}", nextw, t1);
                    transision_w_i.push(index)
                }else{
                    transision_w_i.push(dummy)
            ...
            transition_w.push(transision_w_i)
        }
        transition.push(transition_w)
    }
    let N = (0..W).map(|x| state[x].len()).fold(0, |m,x| m.max(x));
    let mut dp = vec![vec![vec![ModInt::<_, Mod10>::new(0); N]; W]; H+1];
    ...
    for h in 0..H{
        for w in 0..W{
            ...
            for k in 0..state[w].len(){
                if dp[h][w][k].value() == 0{continue;}
                if transition[w][k][0] != dummy{
                    dp[nexth][nextw][transition[w][k][0]] = dp[nexth][nextw][transition[w][k][0]] + dp[h][w][k];
                    // if h == 1 && w == 1{
                    //     eprintln!("zero {:b} {:b}", state[w][k], state[nextw][transition[w][k][0]]);
                    // }

                }
                if transition[w][k][1] != dummy && C[h][w] == b'.'{
                    dp[nexth][nextw][transition[w][k][1]] = dp[nexth][nextw][transition[w][k][1]] + dp[h][w][k];
                    // if h == 1 && w == 1{
                    //     eprintln!("one {:b} {:b}", state[w][k], state[nextw][transition[w][k][1]]);
                    // }

                    // if dp[h][w][k].value() > 0{
                    //     eprintln!("{} {} {:b}", h, w, state[w][k]);
                    // }
                }
            }
        }
    ...
    // for i in 0..H{
    //     for j in 0..W{
    //         println!("i{} j{} {}",i,j, dp[i][j].iter().fold(ModInt::new(0), |sum,&x| sum+x));
    //     }
    // }
    println!("{}", dp[H][0].iter().fold(ModInt::new(0), |sum,&x| sum+x));
}

fn judge(W: usize, w: usize, x: usize) -> bool{
    if W == 1{
        if x&1 > 0{
            return false
        }else{
...
// w < W
fn pre_judge(W: usize, w: usize, x: usize) -> bool{
    if W == 1{return true}
    let t = 1 << W;
    if w == 0{
        if x&t > 0 && x&(t >> 1) > 0{
            return false
        }else if x&(t >> 1) > 0 && x&1 > 0{
            return false
        }else if x&1 > 0 && x&t > 0{
            return false
        }
...
// 1 -> exist
// 近いほうが大きい
fn make_non_one_continuous(W: usize) -> Vec<Vec<usize>>{
    let mut ret = vec![];
    ret.push(vec![]);
    ret.push(vec![0,1]);
    for i in 1..W{
...
// fn dfs(map: &mut HashMap<(usize, usize), (usize, bool)>, W: usize, position: usize, depth: usize, str: usize){
//     let sx = position/W;
//     let sy = position%W;
//     if depth == W+1{
//         let index = 
