...
fn main() {
    ...
    let mut count: Vec<Vec<i64>> = vec![vec![0;26];N+1];
    let a = b'a';
    for i in (0..N).rev(){
    ...
    let mut index = 0;
    let mut ans_bytes = Vec::new();
    for i in 0..K{

        let mut p = 0;
        for j in 0..(26 as u8) {
            if count[index][j as usize] - count[N-(K-1-i)][j as usize] > 0{
                p = j;
                break
            }
        }
        for j in index..(N-(K-i-1)){
            if S[j] - a == p{
                index = j+1;
                ans_bytes.push(S[j]);
                break
    ...
    let ans: String = ans_bytes.iter().map(|&s| s as char).collect::<String>();
    println!("{}", ans);
}
