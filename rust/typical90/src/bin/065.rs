...
fn main() {
    ...
    let mut blue = vec![ModInt::<_, Mod9>::new(0);K-X];
    let mut red = vec![ModInt::<_, Mod9>::new(0);K-Y];
    let mut green = vec![ModInt::<_, Mod9>::new(0); K-Z];
    let factorial = get_factorial(200_000);
    let inversed_factorial = get_inversed_factorial(200_000);
    for b in (K-X)..=B{
        blue.push(factorial[B]*inversed_factorial[b]*inversed_factorial[B-b]);
    }
    for r in (K-Y)..=R{
        red.push(factorial[R]*inversed_factorial[r]*inversed_factorial[R-r]);
    }
    for g in (K-Z)..=G{
        green.push(factorial[G]*inversed_factorial[g]*inversed_factorial[G-g]);
    }
    let br = convolution(&blue, &red);
    let brg = convolution(&br, &green);
    println!("{}", brg[K]);

    
}

fn get_factorial(n: usize) -> Vec<static_prime_modint::ModInt<usize, static_prime_modint::Mod9>>{
    let mut ret = vec![static_prime_modint::ModInt::<_, Mod9>::new(1);1];
    for i in 1..=n{
...
fn get_inversed_factorial(n: usize) -> Vec<static_prime_modint::ModInt<usize, static_prime_modint::Mod9>>{
    let mut ret = vec![static_prime_modint::ModInt::<_, Mod9>::new(1);1];
...
fn get_inversed(n: usize) -> Vec<static_prime_modint::ModInt<usize, static_prime_modint::Mod9>>{
    let mut ret = vec![static_prime_modint::ModInt::<_, Mod9>::new(1);2];
    let Q: usize = 998_244_353;
    for i in 2..=n{
