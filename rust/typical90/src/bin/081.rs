...
fn main() {
    ...
    let five_thousand = 5000;
    let mut height = vec![vec![];five_thousand+1];
    for (i, &(a,_)) in AB.iter().enumerate(){
        height[a].push(i);
    }
    let mut ans = 0;
    let mut queue = VecDeque::new();
    for k in 1..=five_thousand{
        for &i in &height[k]{
            queue.push_back(i);
        }
        if K+1 <= k{
            for _ in 0..height[k-(K+1)].len(){
                queue.pop_front();
            }
        }
        let mut weight = vec![0;five_thousand+1];
        for &i in &queue{
            weight[AB[i].1] += 1;
        }
        let mut now = 0;
        for l in 1..=five_thousand{
            now += weight[l];
            if K+1 <= l{
                now -= weight[l-(K+1)]
            }
