...
fn main() {
    ...
    let strong_connected_components = get_strong_connected_components(&edges);
    let ans = strong_connected_components
        .iter()
        .map(|x| x.len())
        .filter(|&x| x > 0)
        .map(|x| x * (x - 1) / 2)
...
fn get_strong_connected_components(edges: &Vec<Vec<usize>>) -> Vec<Vec<usize>> {
    let N = edges.len();
    ...
    for v in 0..N {
        ...
        let mut part_of_order = Vec::new();
        way_back_dfs(&edges, v, &mut visited, &mut part_of_order);
        for w in part_of_order.iter() {
            order.push(*w);
        }
    }
    let mut reversed_edges = vec![Vec::new(); N];
    for v in 0..N {
        for &w in &edges[v] {
            reversed_edges[w].push(v);
        }
    }
    let mut visited = vec![false; N];
    let mut ret = Vec::new();
    for number in order.iter().rev(){
        if visited[*number] {
            continue;
        }
        ret.push(dfs(&reversed_edges, *number, &mut visited));
    }
    ret
}
...
fn dfs(edges: &Vec<Vec<usize>>, start: usize, visited: &mut Vec<bool>) -> Vec<usize> {
