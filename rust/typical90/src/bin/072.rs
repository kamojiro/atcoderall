...
fn main() {
    ...
    let mut ans = 0;
    for start in 0..hw{
        let (startx, starty) = (start/W, start%W);
        if C[startx][starty] == b'#'{continue;}
        let mut graph = vec![vec![vec![false;W]; H]; 1<<hw];
        graph[1<<start][startx][starty] = true;
        let mut queue = vec![(1<<start, startx, starty)];
        while let Some((p,x,y)) = queue.pop(){
            if adjacent((x,y), (startx,starty)){
                ans = ans.max(count(p));
            }
            if x > 0{
                if C[x-1][y] == b'.' {
                    let q = (x-1)*W+y;
                    let z = 1<<q;
                    if p & z == 0 && !graph[p+z][x-1][y]{
                        graph[p+z][x-1][y] = true;
                        queue.push((p+z,x-1,y));
                    }
                }
            }
            if x < H-1{
            ...
            if y > 0{
                if C[x][y-1] == b'.' {
                    let q = x*W+y-1;
                    let z = 1<<q;
                    if p & z == 0 && !graph[p+z][x][y-1]{
                        graph[p+z][x][y-1] = true;
                        queue.push((p+z,x,y-1));
                    }
                }
            }
            if y < W-1{
...
fn adjacent(x: (usize,usize), y: (usize,usize)) -> bool{
    if (x.0 == y.0) && ((x.1 as i64) - (y.1 as i64)).abs() == 1{
        true
    }else if (x.1 == y.1) && ((x.0 as i64) - (y.0 as i64)).abs() == 1{
        true
    }else{
        false
    }
