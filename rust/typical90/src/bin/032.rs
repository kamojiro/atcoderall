...
fn main() {
    ...
    let mut non_friendly = vec![vec![false; N];N];
    for (x,y) in XY{
        non_friendly[x-1][y-1] = true;
        non_friendly[y-1][x-1] = true;
    }
    let mut ans = std::u64::MAX;
    'order: for order in (0..N).permutations(N){
        for i in 0..(N-1){
            if non_friendly[order[i]][order[i+1]]{
                continue 'order;
            }
        }
        let pans = (0..N).map(|i| A[order[i]][i]).fold(0, |sum, x| sum+x);
        if pans < ans{
            ans = pans;
        }
    }
