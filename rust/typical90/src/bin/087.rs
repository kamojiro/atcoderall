...
fn main() {
    ...
    let mut B = make_first_state(&A, P+1);
    floyd_warshall(&mut B);
    let x_infinity = count_under_P(&B, P);
    if x_infinity == K{
        println!("Infinity");
        return;
    }else if x_infinity > K{
        println!("0");
        return;
    }
    let mut B = make_first_state(&A, 1);
    floyd_warshall(&mut B);
    let x_1 = count_under_P(&B, P);
    if x_1 < K{
        println!("0");
        return;
    }
    ...
    while r-l > 1{
        let m = (l+r)/2;
        let mut B = make_first_state(&A, m);
        floyd_warshall(&mut B);
        if count_under_P(&B, P) > K{
            l = m
        }else{
            r = m
        }
    ...
    let endx = l;
    // eprintln!("{} {}", startx, endx);
    if startx > endx{
        println!("0")
    }else if startx == endx{
        let mut B = make_first_state(&A, startx);
        floyd_warshall(&mut B);
        if count_under_P(&B, P) == K{
            println!("1")
        }else{
...
fn floyd_warshall(distance: &mut Vec<Vec<usize>>){
    let N = distance.len();
    for k in 0..N{
        for i in 0..N{
            for j in 0..N{
...
fn make_first_state(A: &Vec<Vec<i64>>, X: usize) -> Vec<Vec<usize>>{
    let N = A.len();
    let mut ret = vec![vec![X;N];N];
    for i in 0..N{
        for j in 0..N{
            if A[i][j] >= 0{
                ret[i][j] = A[i][j] as usize;
            }
        }
    }
    ret
...
fn count_under_P(distance: &Vec<Vec<usize>>, P: usize) -> usize{
    let mut ret = 0;
    let N = distance.len();
    for i in 0..(N-1){
        for j in (i+1)..N{
