...
fn main() {
    ...
    let mut B: Vec<usize> = B.iter().map(|&x| x).collect();
    B.sort();
    let mut numbering = HashMap::new();
    for (i, &b) in B.iter().enumerate(){
        numbering.insert(b, i+1);
    }
    for i in 0..N{
        A[i] = numbering[&A[i]];
    }
    // eprintln!("{:?}", A);
    let b = 1000_000;
    let mut segtree = SegmentTree::new(b,|| 0, |x,y| x+y);
    let mut cl = vec![0; N];
    let mut index = N;
    let mut inversion = 0;
    A.insert(0, 0);
    let mut V = vec![0; b];
    for r in (0..N).rev(){
        // eprintln!("r {} {}", r, inversion);
        while inversion <= K{
            // index
            // eprintln!("{} {} inversion {}", index, r, inversion);
            if index == N+1{
                break
            }
            let t = A[index];
            // eprintln!("{} inversion {} + {}",index, inversion, segtree.query(0..t));
            inversion += segtree.query(0..t);
            V[t] += 1;
            segtree.update(t, V[t]);
            if index == 0{
                index = N+1
            }else{
                index -= 1;
        ...
        // eprintln!("{} {}", r, index);
        cl[r] = if index == N+1{0} else{index+1};
        // eprintln!("{} - {}",inversion, segtree.query((A[r+1]+1)..b));
        inversion -= segtree.query((A[r+1]+1)..b);
        V[A[r+1]] -= 1;
        segtree.update(A[r+1], V[A[r+1]]);
    }
    // eprintln!("{:?}", cl);
    // println!("{:?}", cl);
    let mut dp = vec![ModInt::<_, Mod10>::new(0); N+1];
    dp[0] = ModInt::new(1);
    let mut acc = vec![ModInt::<_, Mod10>::new(0); N+2];
    acc[1] = ModInt::new(1);
    for i in 0..N{
        dp[i+1] = acc[i+1] - acc[cl[i]];
