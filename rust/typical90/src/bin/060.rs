...
fn main() {
    ...
    let (mut memo, pop) = memorized_longest_increasing_subsequence(&A);
    eprintln!("{:?}", memo);
    eprintln!("{:?}", pop);
    A.reverse();
    let mut ans = memo.len()-1;
    let mut dp = vec![0];
    for i in 0..N{
        let a = A[i];
        // eprintln!("{:?}", dp);
        ...
        // eprintln!("dp{:?} memo{:?}",dp,memo);
        // eprintln!("{} {}", memo.len(), memo[memo.len()-1].len());
        // eprintln!("{:?}", memo[memo.len()-1]);
        let mut ians = dp.len()-1 + memo.len()-1;
        if dp[dp.len()-1] == memo[memo.len()-1][memo[memo.len()-1].len()-1]{
            if ians > 0{
                ians -= 1;                
            }
        }
...
fn memorized_longest_increasing_subsequence(A: &Vec<usize>) -> (Vec<Vec<usize>>, Vec<usize>){
    let mut dp = vec![vec![0]; 1];
    let mut pop = Vec::new();
    for &a in A{
        if dp[dp.len()-1][dp[dp.len()-1].len()-1] < a{
            dp.push(vec![a]);
            pop.push(dp.len()-1);
            continue;
        }
        ...
        while r-l > 1{
            let m = (l+r)/2;
            if dp[m][dp[m].len()-1] < a{
                l = m
            }else{
                r = m
            }
        }
        if a < dp[l+1][dp[l+1].len()-1]{
            pop.push(l+1);
            dp[l+1].push(a)
        }else{
            pop.push(0)
