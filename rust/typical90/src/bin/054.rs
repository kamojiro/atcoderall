...
fn main() {
    ...
    for i in 0..M{
        for &r in &R[i]{
            edges[r-1].push((N+i,1));
            edges[N+i].push((r-1,0));
        }
    }
    let ans = zero_one_bfs(&edges, 0);
    for i in 0..N{
...
fn zero_one_bfs(edges: &Vec<Vec<(usize, usize)>>, start: usize) -> Vec<usize>{
    ...
    while let Some(v) = queue.pop_front(){
        for &(w, s) in &edges[v]{
            if dist[w] < std::usize::MAX{continue;}
            dist[w] = dist[v] + s;
            if s == 0{
                queue.push_front(w);
            }else{
