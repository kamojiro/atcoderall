...
fn main() {
    ...
    let mut exist_info = vec![false; N+1];
    let mut alternate_sum = vec![0;N+1];
    for &(t,x,_,v) in &TXYV{
        if t == 1{continue;}
        exist_info[x] = true;
        alternate_sum[x] = ((x as i64)%2*2-1)*v;
    }
    let mut now = false;
    for i in 1..=N{
        if now{
            if exist_info[i]{
                alternate_sum[i] += alternate_sum[i-1];
            }else{
                now = false;
            }
        }else{
    ...
    for &(t,x,y,v) in &TXYV{
        if t == 0{
            ...
            if tree.equiv(x,y){
                ...
                let p = x.min(y);
                let q = x.max(y);
                let f = (1-(q as i64)%2*2)*(alternate_sum[q-1] - alternate_sum[p-1]);
                if x < y{
                    if (q-p)%2 == 1{
                        println!("{}", f-(v as i64))
                    }else{
