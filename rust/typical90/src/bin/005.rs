...
// dp[i+1][x] = \sum_{0 <= y < B, 1<= k <= K, (10y+p)%B=x}dp[i][y]
// 基本方針: 行列累乗
// 考察:
// 更新される場所が思ったよりも少ない気がする
// 行列累乗だとあんまり関係ない?
...
fn main() {
    ...
    // let mut ten_powers = vec![ModInt::<_, Mod10>::new(1); 61];
    // for i in 1..60 {
    //     ten_powers[i] = ten_powers[i-1].pow(2);
    // }

    let mut dp = vec![vec![ModInt::<_, Mod10>::new(0); B]; 61];
    dp[0][0] = ModInt::new(1);
    for c in &C{
        dp[1][c%B] = dp[1][c%B] + ModInt::new(1);
    }
    ...
    for i in 2..61{
        for j in 0..B{
            for k in 0..B{
                dp[i][(j*zero + k)%B] = dp[i][(j*zero + k)%B]+dp[i-1][j]*dp[i-1][k];
            }
        }
        zero = zero*zero%B;
    }
    ...
    let mut dp_ans = vec![vec![ModInt::<_, Mod10>::new(0); B]; 61];
    dp_ans[0][0] = ModInt::new(1);
    let mut times = N;
    zero = 10%B;
    for i in 1..61{
        if times%2 == 1{
            for j in 0..B{
                for k in 0..B{
                    // println!("{:?}", dp[i-1]);
                    // println!("{} {} {} {} {}",(j*zero+k)%B,  j, k, dp_ans[i-1][j].value(), dp[i][k].value());
                    dp_ans[i][(j*zero + k)%B] = dp_ans[i][(j*zero + k)%B] + dp_ans[i-1][j] * dp[i][k];
                }
            }
        }else{
            for j in 0..B{
                dp_ans[i][j] = dp_ans[i-1][j];
            }
        }
        // println!("{} {:?} {}", i, dp_ans[i], zero);
        times /= 2;
        if times == 0{
            println!("{}", dp_ans[i][0].value());
            return
        }
        zero = zero * zero %B;
