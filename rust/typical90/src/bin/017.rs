...
fn main() {
    ...
    let mut ans: usize = M*(M-1)/2;
    let mut duplicate = vec![0;N+1];
    debug_eprintln!("nnnn {:?}", LR);
    debug_eprintln!("a {}", ans);
    for &(l,r) in &LR{
        duplicate[l] += 1;
        duplicate[r] += 1;
    }
    for i in 1..=N{
        ans -= ((duplicate[i] as i64)*((duplicate[i] as i64)-1)/2 )as usize;
    }
    debug_eprintln!("a1 {}", ans);
    ...
    debug_eprintln!("a2 {}", ans);
    let mut num = vec![0;N+1];
    let mut segt = SegmentTree::new(N+1, || 0, |a,b| a+b);
    // for &(l,r) in &LR{
    //     ans -= segt.query((l+1)..r);
    //     num[l] += 1;
    //     segt.update(l, num[l]);
    // }
    let mut count = 0;
