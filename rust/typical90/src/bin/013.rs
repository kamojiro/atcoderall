...
fn main() {
    ...
    let distance_from_start = dijkstra(0, N, &edges);
    let distance_from_goal = dijkstra(N-1, N, &edges);
    for k in 0..N{
        println!("{}", distance_from_start[k] + distance_from_goal[k]);
    }
}

use std::{cmp::Reverse, collections::BinaryHeap};
fn dijkstra(start: usize, N: usize, edges: &Vec<Vec<(usize, u64)>>) -> Vec<u64>{
    let mut queue = BinaryHeap::new();
    let mut ret = vec![std::u64::MAX; N];
    queue.push((Reverse(0), start));
    while let Some((Reverse(time), city)) = queue.pop(){
        if ret[city] < std::u64::MAX{
            continue;
        }
        ret[city] = time;
        for &(next, will) in &edges[city]{
            if ret[next] < std::u64::MAX{continue;}
            queue.push((Reverse(time + will), next));
        }
    }
    ret
}
