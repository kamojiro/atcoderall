...
fn main() {
    ...
    for _ in 0..Q{
        ...
        V.sort_by(|a,b| dfs_order[*a].cmp(&dfs_order[*b]));
        queries.push(V);
    }
    ...
    for i in 0..queries.len(){
        ...
        let k = queries[i].len();
        for j in 0..k{
            ans += lca.distance(queries[i][j], queries[i][(j+1)%k]);
        }
        println!("{}", ans/2);
    }
...
fn dfs(n: usize, start: usize, edges: &Vec<Vec<usize>>) -> Vec<usize>{
