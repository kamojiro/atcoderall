...
fn main() {
    ...
    let mut kakko: Vec<BTreeSet<String>> = vec![BTreeSet::new(); 1];
    kakko[0].insert("".to_string());
    let l = "(".to_string();
    let r = ")".to_string();
    for i in 1..=10{
        let mut set:BTreeSet<String> = BTreeSet::new();
        for middle in &kakko[i-1]{
            let x = middle.to_string().clone();
            set.insert([l.clone(), x, r.clone()].concat());
        }
        for j in 1..i{
            for front in &kakko[j]{
                for back in &kakko[i-j]{
                    set.insert([front.to_string(), back.to_string()].concat());
                }
            }
        }
        kakko.push(set);
