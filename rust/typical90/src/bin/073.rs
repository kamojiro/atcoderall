...
fn main() {
    ...
    let mut order = bfs_order(&edges, 0);
    order.reverse();
    // 0 -> only a, 1 -> only b, 2 -> a and b
    let mut dp = vec![vec![ModInt::<_, Mod10>::new(0);3];N];
    ...
    for v in order{
        ...
        if C[v] == 'a'.to_string(){
            let mut s = vec![ModInt::new(1);2];
            let mut t = ModInt::new(1);
            for &w in &edges[v]{
                if !visited[w]{continue;}
                s[0] *= dp[w][0] + dp[w][2];
                s[1] *= dp[w][0] + dp[w][1] + ModInt::new(2)*dp[w][2];
                t *= dp[w][0] + dp[w][2];
            }
            dp[v][0] = s[0];
            dp[v][2] = s[1] - t;
...
fn bfs_order(edges: &Vec<Vec<usize>>, start: usize) -> Vec<usize>{
