...
fn main() {
    ...
    let Z = K;
    let mut primes: HashMap<usize,usize> = HashMap::new();
    let L = (K as f64).sqrt() as usize;
    for i in 2..=L{
        while K%i == 0{
            let t = *primes.entry(i).or_insert(0) + 1;
            primes.insert(i, t);
            K /= i;
        }
    }
    if K > 1{
        let t = *primes.entry(K).or_insert(0) + 1;
        primes.insert(K, t);
    ...
    for key in primes.keys(){
        ...
        let mut now = 1;
        for _ in 0..=primes[key]{
            multiples.push(now);
            now *= key;
        }
        let mut new_divisor = Vec::new();
        for &x in &divisor{
            for &y in &multiples{
                new_divisor.push(x*y)
            }
        }
        divisor = new_divisor;
    }
    ...
    let mut ans = 0;
    for i in 0..N{
        if Z/divisor[i] < divisor[i]{break}
        for j in i..N{
            let t = divisor[i]*divisor[j];
            if t > Z{break;}
            // eprintln!("{} {} {} {}", divisor[i], divisor[j], t, Z%t);
            if Z%t > 0{continue;}
            // eprintln!("{} {}", divisor[i], divisor[j]);
            if Z/t < divisor[j]{break;}
            ans += 1;
        }
    }
    println!("{}", ans);
