...
fn main() {
    ...
    let mut factorial_table = vec![ModInt::<_, Mod10>::new(1)];
    let mut factorial_inv_table = vec![ModInt::<_, Mod10>::new(1)];
    for i in 1..=N {
    ...
    for k in 1..=N{
        ...
        for a in 1..=((N-1)/k+1){
            ans = ans + factorial_table[N-(a-1)*(k-1)]*factorial_inv_table[a]*factorial_inv_table[N-(a-1)*(k-1)-a];
        }
        println!("{}", ans.value());
    }
