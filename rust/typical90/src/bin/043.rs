...
fn main() {
    ...
    queue.push((Reverse(0), rs,cs, 1));
    let mut dist = vec![vec![vec![std::usize::MAX; 2]; W]; H];
    let mut visited = vec![vec![vec![false; 2]; W]; H];
    dist[rs][cs][0] = 0;
    dist[rs][cs][1] = 0;
    while let Some((Reverse(x),r,c,p)) = queue.pop(){
        if visited[r][c][p]{continue;}
        visited[r][c][p] = true;
        dist[r][c][p] = x;
        let dr = if p == 0 {0}else{1};
        let dc = if p == 1 {0}else{1};
        if r > 0{
            if !visited[r-1][c][0] && (S[r-1][c] == b'.'){
                queue.push((Reverse(x+dr), r-1,c, 0))
            }
        }
        if r < H-1{
            if !visited[r+1][c][0] && (S[r+1][c] == b'.'){
        ...
        if c > 0{
            if !visited[r][c-1][1] && (S[r][c-1] == b'.'){
                queue.push((Reverse(x+dc), r, c-1,1))
            }
        }
        if c < W-1{
            if !visited[r][c+1][1] && (S[r][c+1] == b'.'){
