...
fn main() {
    ...
    let path_cost = bfs(g, &edges); 
    let diameter = path_cost.iter().max().unwrap();
    println!("{}", diameter+1);
}

use std::collections::VecDeque;
fn bfs(start: usize, edge: &Vec<Vec<usize>>) -> Vec<usize>{
    let mut distance = vec![std::usize::MAX; edge.len()];
    distance[start] = 0;
    let mut queue: VecDeque<usize> = VecDeque::new();
    queue.push_back(start);
    while let Some(v) = queue.pop_front(){
        for w in &edge[v]{
            if distance[*w] < std::usize::MAX{
                continue
            }
...
fn dfs(start: usize, edge: &Vec<Vec<usize>>) -> Vec<usize>{
    let mut distance = vec![std::usize::MAX; edge.len()];
    distance[start] = 0;
