...
fn main() {
    ...
    let mut utree = UnionFind::new(H*W);
    ...
    for _ in 0..Q{
        ...
        if q == 1{
            ...
            rb -= 1;
            cb -= 1;
            // let &ra = query.get(1).unwrap();
            // let &ca = query.get(2).unwrap();
            // let &rb = query.get(1).unwrap();
            // let &cb = query.get(2).unwrap();
            // println!("{} {} {} {}", ra, ca, rb,cb);
            if board[ra][ca] && board[rb][cb] && utree.equiv(ra*W+ca, rb*W+cb){
                println!("Yes")                
            }else{
                println!("No")
            }
