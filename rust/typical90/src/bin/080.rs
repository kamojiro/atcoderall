...
fn main() {
    ...
    let mut pow: Vec<i128> = Vec::new();
    for i in 0..=D{
        pow.push(1<<i as i128)
    }
    for i in 1..(1 << N){
        let counti = (0..=D).filter(|&x| ((i as i128) & pow[x]) > 0).count();
        let q = (0..=D).filter(|&x| ((i as i128) & pow[x]) > 0).fold(0, |or, x| or | A[x]);
        let counta = (0..D).filter(|&x| (q & pow[x]) > 0).count();
        if counti%2 == 1{
            ans -= pow[D-counta]
        }else{
            ans += pow[D-counta]
