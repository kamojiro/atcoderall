...
fn main() {
    ...
    let two = ModInt::new(2).pow(1_000_000_005);
    for i in 0..=18{
        if r <= L{
            ;
        }else if L < l{
            if r <= R{
                ans = ans + ModInt::new(i+1)*ModInt::new((r-l) as usize)*ModInt::new((l+r-1) as usize)*two
            }else if l <= R{
                ans = ans + ModInt::new(i+1)*ModInt::new((R+1-l) as usize)*ModInt::new((l+R) as usize)*two
            }else{
               ;
            }
        }else{
