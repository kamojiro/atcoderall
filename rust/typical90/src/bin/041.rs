...
fn main() {
    ...
    // 凸包
    let mut upper_convex_hull = Vec::new();
    let mut lower_convex_hull = Vec::new();
    for z in &XY{
        if upper_convex_hull.len() >= 2{
            while cross_product(upper_convex_hull[upper_convex_hull.len()-2], upper_convex_hull[upper_convex_hull.len()-1], *z) <= 0{
                upper_convex_hull.pop();
                if upper_convex_hull.len() < 2{break}
            }
        }
        upper_convex_hull.push(*z);
        if lower_convex_hull.len() >= 2{
            while cross_product(lower_convex_hull[lower_convex_hull.len()-2], lower_convex_hull[lower_convex_hull.len()-1], *z) >= 0{
                lower_convex_hull.pop();
                if lower_convex_hull.len() < 2{break}
            }
        }
        lower_convex_hull.push(*z);
    }
    // 合体
    let mut convex_hull = upper_convex_hull;
    if convex_hull[convex_hull.len()-1] == lower_convex_hull[lower_convex_hull.len()-1]{
        lower_convex_hull.pop();
    }
    lower_convex_hull.reverse();
    convex_hull.append(&mut lower_convex_hull);
    ...
    for i in 0..(convex_hull.len()-1){
        let p = convex_hull[i];
        let q = convex_hull[i+1];
        b += gcd(p.0 - q.0, p.1 - q.1);
    }
    // eprintln!("S*2: {}, b: {}", S, b);
    // eprintln!("{}", (S + b)/2 + 1);
    println!("{}", ((S + b)/2 + 1) - (N as i64));
}

fn cross_product(p: (i64,i64), q: (i64,i64), r: (i64, i64)) -> i64{
    let s = (q.0 - p.0, q.1 - p.1);
    let t = (r.0 - q.0, r.1 - q.1);
    s.0 * t.1 - s.1 * t.0
}

fn get_area(p: (i64,i64), q: (i64,i64)) -> i64{
    (p.0 - q.0) * (p.1 + q.1)
}
...
where
    T: PrimInt,
{
    if a < T::zero() {return gcd(T::zero().sub(a),b)}
    if b < T::zero() {return gcd(a,T::zero().sub(b))}
    if a < b {
        return gcd(b, a);
    } else if b == T::zero() {
        return a;
