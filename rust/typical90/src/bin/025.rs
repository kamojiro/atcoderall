...
fn main() {
    ...
    dfs("".to_string(),0,&mut range);

    let mut ans = 0;
    for fm in range{
        if fm == "".to_string(){continue}
        if check(fm, B, N){
            ans += 1
        }
    }
...
fn dfs(x: String, next: usize, range: &mut Vec<String>){
    if x.len() > 11{return}
    let z = x.clone();
    range.push(z);
    for i in next..=9{
        let mut y = x.clone();
        y.push(std::char::from_digit(i as u32, 10).unwrap());
        dfs(y,i,range);
    }
} 
 
fn check(fm: String, B : usize, N: usize,) -> bool{
    let mut x: usize = fm.chars().map(|x| x as usize -48).fold(1, |product, i| product*i);
    x += B;

    if x > N{
        return false
    }
    let mut xnums = vec![0;10];
    let mut ynums = vec![0;10];
    fm.chars().map(|x| x as usize -48).for_each(|x| ynums[x] += 1);
    while x > 0 {
        xnums[x%10] += 1;
        x /= 10
    }
    for i in 0..10{
        if xnums[i] != ynums[i]{return false}
    }
    true
}
