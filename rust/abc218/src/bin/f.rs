...
fn main() {
    ...
    let l = prev_bfs(0, &edges, &mut prev);
    let mut visited = Vec::new();
    let mut v = N-1;
    while prev[v] != N{
        visited.push((prev[v],v));
        v = prev[v];
    }
    let mut ans = vec![l; M];
    for &vw in &visited{
...
fn bfs(s: usize, edges: &Vec<Vec<usize>>, st: (usize,usize)) -> usize{
    let n = edges.len();
    let mut ret = vec![n; n];
    ret[s] = 0;
...
fn prev_bfs(s: usize, edges: &Vec<Vec<usize>>, prev: &mut Vec<usize>) -> usize{
    let n = edges.len();
    let mut ret = vec![n; n];
    ret[s] = 0;
    let mut queue = VecDeque::new();
