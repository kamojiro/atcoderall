...
fn main() {
    ...
    dfs(0, 0, &edges, &C, &mut count,&mut ans);
    for &a in &ans{
        println!("{}", a)
    }
}

fn dfs(v: usize, prev: usize, edges: &Vec<Vec<usize>>, C: &Vec<usize>, count: &mut Vec<usize>, ans: &mut Vec<usize>) -> usize{
    let mut sz = 1;
    let c = count[C[v]];
    if v == 0{
        for &w in &edges[v]{
            if w == prev{continue;}
            let q = count[C[v]];
            let d = dfs(w,v,edges,C,count,ans);
            sz += d;
            let k = d - count[C[v]] + q;
            ans[C[v]] -= k*(k+1)/2;
        }
