...
fn main() {
    ...
    let mut B = A.iter().enumerate().map(|(i, &a)| (a, (i+1) as i64)).collect_vec();
    ...
    for x in 0..=N{
        for y in 0..=N{
            ...
            let (a, i) = B[x+y-1];
            if x > 0{
                dpxy = dpxy.max(dp[x-1][y] + a*(i - (x as i64)));
            }
            if y > 0{
                dpxy = dpxy.max(dp[x][y-1] + a*(((N+1-y) as i64) - i));
            }
            dp[x][y] = dpxy;
        }
    }
    // eprintln!("{:?}", dp);
    let ans = (0..=N).map(|i| dp[i][N-i]).max().unwrap();
    println!("{}", ans);
}
