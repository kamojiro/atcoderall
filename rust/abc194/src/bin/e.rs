...
fn main() {
    ...
    for i in 0..N{
        if count[i] == 0{
            btree.insert(i as u64);
        }
    }
    btree.insert(N as u64);
    let mut ans = N as u64;
    if ans > *btree.iter().next().unwrap(){
        ans = *btree.iter().next().unwrap();
    }
    for i in M..N{
        count[A[i] as usize] += 1;
        if count[A[i] as usize] == 1{
            btree.remove(&A[i]);
        }
        count[A[i-M] as usize] -= 1;
        if count[A[i-M] as usize] == 0{
            btree.insert(A[i-M]);
        }
        if ans > *btree.iter().next().clone().unwrap(){
            ans = *btree.iter().next().clone().unwrap();
        }
        // let t = &btree.iter().next().clone();
        // if t < ans{
        //     ans = t;
        // }
        // if ans > t{
