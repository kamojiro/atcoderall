...
fn main() {
    ...
    // println!("{}", lower_bound(&A, 3)) -> 0
    let z = 1_000_000_000_000_000_000 + 1_000_000;
    for k in queries{
        let mut l = 0;
        let mut r = z;
        while r - l > 1{
            let m = (l+r)/2;
            let p = lower_bound(&A, m);
            let t = m -  (p as u64);
            if t < k{
                l = m
            }else{
    ...
    // eprintln!("{:?}", A);
    // for i in 0..10usize{
    //     eprintln!("{} {}",i,  lower_bound(&A, i as u64));
    // }
    // eprintln!("{}", lower_bound(&A, 3))
}

fn lower_bound(A: &Vec<u64>, a: u64) -> usize {
    let N = A.len();
    if a < A[0] {return 0}
    if A[N-1] < a{return N}
    let mut l = 0;
