...
fn main() {
    input!{
        ...
        W: usize,
        N: usize,
        CDST: [(Usize1, Usize1, usize, usize); N],
        G: [Bytes; H],
    }
    if H==1 && W==1{
        println!("{}", N);
    ...
    // N+1 = start
    let mut distance = vec![vec![std::usize::MAX; N+1]; N+1];
    for i in 0..(N+1){
        let mut queue = VecDeque::new();
        if i < N{
            queue.push_back((CDST[i].0, CDST[i].1))
        }else{
            queue.push_back((0, 0))
        }
        let mut path = vec![vec![std::usize::MAX; W]; H];
        path[queue[0].0][queue[0].1] = 0;
        while let Some((x, y)) = queue.pop_front(){
            if x > 0{
                if G[x-1][y] == b'.' && path[x-1][y] == std::usize::MAX{
            ...
            if y > 0{
                if G[x][y-1] == b'.' && path[x][y-1] == std::usize::MAX{
        ...
        distance[i][N] = path[0][0];
        for j in 0..N{
            distance[i][j] = path[CDST[j].0][CDST[j].1];
        }
    }
    let mut bombed = vec![vec![std::usize::MAX; N+1]; 1<<N];
    ...
    for i in 0..N{
        ...
        if d <= t{
            if s == t{
                if d%2 == s%2{
                    queue.push((Reverse(s), 1<<i, i));
                }
                continue
            }
            if d <= s{
                let z = if d%2 == s%2{0}else{1};
                queue.push((Reverse(s+z), 1<<i, i))
            }else{
                queue.push((Reverse(d), 1<<i, i))
            }
        }
    }
    while let Some((Reverse(time), p, n)) = queue.pop(){
        if bombed[p][n] < std::usize::MAX{
            continue
        }
        bombed[p][n] = time;
        for i in 0..N{
            ...
            if time+d <= t{
                if s == t{
                    if (time+d)%2 == s%2{
                        queue.push((Reverse(s), p+(1<<i), i));
                    }
                    continue;
                }
                if time+d <= s{
                    let z = if (time+d)%2 == s%2{0}else{1};
                    queue.push((Reverse(s+z), p+(1<<i), i))
                }else{
                    queue.push((Reverse(time+d), p+(1<<i), i))
                }
            }
        }
    }
