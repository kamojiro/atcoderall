...
fn main() {
    ...
    let mut heads = vec![vec![]; 1000_000];
    for (i,s) in S.iter().enumerate(){
        let p = &s[0..3].iter().map(|&x| x).collect();

        heads[to_usize(p)].push(i)
    }
    let mut edges = vec![vec![]; N];
    let mut inverse_edges = vec![vec![]; N];
    let mut source_count = vec![0; N];
    for (i,s) in S.iter().enumerate(){
        let p = &s[(s.len()-3)..s.len()].iter().map(|&x| x).collect();
        // eprintln!("{:?}", p);
        eprintln!("{}", to_usize(p));
        for &index in &heads[to_usize(p)]{
            edges[i].push(index);
            source_count[i] += 1;
            inverse_edges[index].push(i);
        }
    }
    // 0 -> Draw, 1 -> win, 2 -> lose
    let mut judge = vec![0; N];
    ...
    while let Some(v) = queue.pop_front(){
        for &w in &inverse_edges[v]{
            if judge[w] > 0{continue;}
            source_count[w] -= 1;
            if judge[v] == 2{
                judge[w] = 1;
                queue.push_back(w);
            }else if source_count[w] == 0{
                judge[w] = 2;
                queue.push_back(w);
            }
        }
...
fn to_usize(S: &Vec<u8>) -> usize{
    let A = 65;
    let mut ret = 0;
    for &s in S{
        ret = ret*100 + (s as usize) - A;
    }
    ret
}
