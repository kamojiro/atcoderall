...
fn main() {
    ...
    if N == 2{
        if (AB[0].0 - AB[1].0).pow(2) + (AB[0].1 - AB[1].1).pow(2) == ((CD[0].0 - CD[1].0) as i64).pow(2) + ((CD[0].1 - CD[1].1) as i64).pow(2){
            println!("Yes")
        }else{
            println!("No")
        }
    ...
    let (a,b) = AB[0];
    let mut ABMAP = vec![vec![false; 41]; 41];
    let z = 20;
    for i in 0..N{
        ABMAP[(AB[i].0-a+z) as usize][(AB[i].1-b+z) as usize] = true;
    }
    for i in 0..N{
        let (c,d) = CD[i];
        'rotation: for p in 0..360{
            let rad = (p as f64)/180.0*PI;
            let (sin,cos) = rad.sin_cos();
            let mut CDMAP = vec![vec![false; 41]; 41];
            for j in 0..N{
                let ( mut fc, mut fd) = CD[j];
                fc -= c;
                fd -= d;
                let rc = cos*fc - sin*fd;
                let rd = sin*fc + cos*fd;
                let rrc = rc.round();
                let rrd = rd.round();
                // if (j == 0) && (p%90==0){
                //     eprintln!("p {} rc{} rrc{} rd{} rrd{}",p, rc, rrc, rd, rrd);
                // }
                if ((rc - rrc).abs() < 1e-1) && ((rd - rrd).abs() < 1e-1){
                    // if (i == 0) && (p == 180){
                    //     eprintln!("{} {}", (rc - rrc).abs(), (rd - rrd).abs());
                    // }
                    if ((rrc as i64).abs() > z) || ((rrd as i64).abs() > z){
                        continue 'rotation;
                    }
                    CDMAP[((rrc as i64) + z) as usize][((rrd as i64) + z) as usize] = true;
                }else{
                    continue 'rotation;
                }
            }
            ...
            for j in 0..41{
                for k in 0..41{
                    // if (i == 0) && (p == 180) && ( 17 <= j) && (j <= 23) && (17 <= k) && (k <= 17){
                    //     println!("{} {} {} {}", (j as i64)-z, (k as i64)-z,ABMAP[j][k], CDMAP[j][k]);
                    // }

                    if ABMAP[j][k] != CDMAP[j][k]{
                        // if (i == 1) && (p == 270){
                        //     eprintln!("{} {} {} {}", j as i64-z, k as i64-z, ABMAP[j][k], CDMAP[j][k])
                        // }
                        continue 'rotation;
                    }
                }
