...
fn main() {
    ...
    // if N == 2{
    //     if (AB[0].0 - AB[1].0).pow(2) + (AB[0].1 - AB[1].1).pow(2) == (CD[0].0 - CD[1].0).pow(2) + (CD[0].1 - CD[1].1).pow(2){
    //         println!("Yes")
    //     }else{
    //         println!("No")
    //     }
    ...
    for i in 0..N{
        ...
        // eprintln!("{} {}", AB[i].0-a+z, AB[i].1-b+z);
    }
    let cos = vec![1,0,-1,0];
    let sin = vec![0,1,0,-1];
    for i in 0..N{
        ...
        'rotation: for p in 0..4{
            let mut CDMAP = vec![vec![false; 41]; 41];
            for j in 0..N{
                let ( mut fc, mut fd) = CD[j];
                fc -= c;
                fd -= d;
                let rc = cos[p]*fc - sin[p]*fd;
                let rd = sin[p]*fc + cos[p]*fd;
                CDMAP[(rc+z) as usize][(rd+z) as usize] = true;
            }
            // if (i == 0) && (p == 180){
            //     eprintln!("{:?}", CDMAP);
            // }
