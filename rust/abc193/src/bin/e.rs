...
fn solve(X: i128, Y: i128, P: i128, Q: i128) -> i128 {
    let mut ans: i128 = std::i128::MAX;
    let (d, _, y) = ext_gcd(2 * (X + Y), P + Q);
    // println!("{} {}",x,y );
    let lcm = 2 * (X + Y) * (P + Q) / gcd(2 * (X + Y), P + Q);
    // println!("lcm: {}", lcm);
    for t1 in X..(X + Y) {
        for t2 in P..(P + Q) {
            if (t1 - t2) % d != 0 {
