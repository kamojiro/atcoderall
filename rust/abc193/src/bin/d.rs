...
fn main() {
    ...
    let mut nums:Vec<i64> = vec![K;10];
    let mut takahashi:Vec<i64> = vec![0;10];
    let mut aoki:Vec<i64> = vec![0;10];
    for i in 0..4{
        nums[S[i] as usize - 48] -= 1;
        takahashi[S[i] as usize - 48] += 1;
        nums[T[i] as usize - 48] -= 1;
        aoki[T[i] as usize - 48] += 1;
    }
    // println!("takahashi {:?}", takahashi);
    // println!("aoki {:?}", aoki);
    // println!("{:?}", nums);
    let mut ans: i64 = 0;
    for i in 1..=9{
        let mut real_takahashi = takahashi.clone();
        real_takahashi[i] += 1;
        for j in 1..=9{
            if i == j{
                if nums[i] < 2{
            ...
            let mut real_aoki = aoki.clone();
            real_aoki[j] += 1;
            // println!("{} {}",i,j );
            // // println!("{:?}{:?}", real_takahashi,real_aoki);
            // println!("{} {}", calc(&real_takahashi),calc(&real_aoki) );
            if calc(&real_takahashi) > calc(&real_aoki){
    ...
    let denominator:f64 = ((9*K-8)*(9*K-9)) as f64;
    // println!("{}", ans);
    println!("{}", (ans as f64)/denominator);
}

fn calc(hand: &Vec<i64>) -> i64 {
    let mut ret: i64 = 0;
    for i in 1..10{
        ret += (i as i64)*(10 as i64).saturating_pow(hand[i] as u32);
    }
    ret
}
