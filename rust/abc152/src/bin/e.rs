...
fn main() {
    ...
    let mut g = vec![0; 1000_001];
    A.iter().for_each(|&x| prime_factors(x, &mut g));
    let mut gg = ModInt::<_, Mod10>::new(1);
    for (i, &a) in g.iter().enumerate(){
        if a > 0{
            gg *= ModInt::new(i).pow(a);
        }
    }
    let mut ans = ModInt::<_, Mod10>::new(0);
    for &a in &A{
        ans += gg*ModInt::<usize, Mod10>::new(a).inverse();
    }
    println!("{}", ans);

}

fn prime_factors(x: usize, g: &mut Vec<usize>){
    let mut y = x;
    let mut d = 2;
    while d*d <= y{
        let mut c = 0;
