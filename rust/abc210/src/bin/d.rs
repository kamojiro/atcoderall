...
fn main() {
    ...
    let mut ans = solveppmm(H, W, C, &A);
    // eprintln!("{}", ans);
    for h in 0..H{
        A[h].reverse();
    }
    ans = ans.min(solveppmm(H, W, C, &A));

    
    println!("{}",ans);
}

fn solveppmm(H: usize, W: usize, C: i64, A: &Vec<Vec<i64>>) -> i64{
    let mut ans = A[0][0] + A[1][0] + C;
    
    let mut minusminus = vec![vec![vec![(1000_000_000_000i64, H,W);2]; W];H];
    for h in 0..H{
        for w in 0..W{
            let mut z = vec![(1000_000_000_000i64, H,W);2];
            if h > 0{
                for i in 0..2{
                    z.push(minusminus[h-1][w][i]);
                }
            }
            if w > 0{
                for i in 0..2{
                    z.push(minusminus[h][w-1][i]);
                }
            }
            z.push((A[h][w]+C*(-(h as i64)-(w as i64)),h,w));
            z.sort();
            minusminus[h][w][0] = z[0];
            minusminus[h][w][1] = z[1];

        }
    }
    for h in 0..H{
        for w in 0..W{
            if (minusminus[h][w][0].1 == h) && (minusminus[h][w][0].2 == w ){
                ans = ans.min(A[h][w] + C*((h as i64)+(w as i64)) + minusminus[h][w][1].0);
            }else{
                ans = ans.min(A[h][w] + C*((h as i64)+(w as i64)) + minusminus[h][w][0].0);
            }
        }
    }
    ans
}
// fn solvepmmp(H: usize, W: usize, C: i64, A: &Vec<Vec<i64>>) -> i64{
//     let mut ans = A[0][0] + A[1][0] + C;
    
//     let mut minusminus = vec![vec![vec![(1000_000_000_000i64, H,W);2]; W];H];
//     let mut minusplus = vec![vec![vec![(1000_000_000_000i64, H,W);2]; W];H];
//     for h in 0..H{
//         for w in 0..W{
//             let mut z = vec![(1000_000_000_000i64, H,W);2];
//             if h > 0{
...
//             if h > 0{
//                 for i in 0..2{
//                     z.push(minusplus[h-1][w][i]);
//                 }
//             }
//             if w > 0{
//                 for i in 0..2{
//                     z.push(minusplus[h][w-1][i]);
//                 }
//             }
//             z.push((A[h][w]+C*(-(h as i64)+(w as i64)),h,w));
//             z.sort();
...
//     for h in 0..H{
//         for w in 0..W{
//             eprintln!("h{} w{} ans{}",h, w, ans);
//             if (minusminus[h][w][0].1 == h) && (minusminus[h][w][0].2 == w ){
//                 ans = ans.min(A[h][w] + C*((h as i64)+(w as i64)) + minusminus[h][w][1].0);
//             }else{
//                 ans = ans.min(A[h][w] + C*((h as i64)+(w as i64)) + minusminus[h][w][0].0);
//             }
//             eprintln!("ppmm h{} w{} ans{}",h, w, ans);
            
//             if (minusplus[h][w][0].1 == h) && (minusplus[h][w][0].2 == w ){
//                 ans = ans.min(A[h][w] + C*((h as i64)-(w as i64)) + minusplus[h][w][1].0);
//             }else{
//                 ans = ans.min(A[h][w] + C*((h as i64)-(w as i64)) + minusplus[h][w][0].0);
//             }
//             eprintln!("pmmp h{} w{} ans{}",h, w, ans);
//         }
//     }
//     ans
// }
