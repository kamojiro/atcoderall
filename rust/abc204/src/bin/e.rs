...
fn main() {
    input!{
        N: usize,
        M: usize,
        ABCD: [(usize, usize, u64, u64); M],
    }
    let mut edges = vec![vec![];N];
    for (a,b,c,d) in ABCD{
        edges[a-1].push((b-1,c,d));
        edges[b-1].push((a-1,c,d));
    }
    ...
    while let Some((Reverse(time), city)) = queue.pop(){
        ...
        for &(next, c,d) in &edges[city]{
            if visited[next]{continue;}
            queue.push((Reverse(good_time(c, d, time)), next));
        }
    }
    if !visited[N-1]{
        println!("-1");
...
fn good_time(c: u64, d: u64, now: u64) -> u64{
    if d == 0{
        return now+c
    }
    if d == 1{
    ...
    while r-l > 1{
        let m = (l+r)/2;
        let t = (d as f64)/((now+m+1) as f64) - (d as f64)/((now+m+1+1) as f64);
        if t > 1.0{
            l = m
        }else{
            r = m
    ...
    now + c + min(d/(now+1+l)+l, d/(now+1+r)+r)
}
