...
fn main() {
    ...
    let mut IRCA = Vec::new();
    for (i,&(r,c,a)) in RCA.iter().enumerate(){
        IRCA.push((i, r, c, a));
    }
    IRCA.sort_by_key(|x| Reverse(x.3));
    let mut ans = vec![0; N];
    let mut rcount: Vec<usize> = vec![0; H];
    let mut ccount: Vec<usize> = vec![0; W];
    let mut nowa = 1_000_000_000_0;
    let mut stack = Vec::new();
    for &(i,r,c,a) in &IRCA{
        if nowa != a{
            nowa = a;
            for &(pr, pc, pans) in &stack{
                if rcount[pr] < pans{
                    rcount[pr] = pans;
                }
                if ccount[pc] < pans{
        ...
        let ians = rcount[r].max(ccount[c])+1;
        ans[i] = ians;
        stack.push((r,c,ians));
    }
    for &a in &ans{
