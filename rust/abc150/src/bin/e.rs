...
fn main() {
    ...
    let mut ans = ModInt::<_,Mod10>::new(2).pow(N-1)*ModInt::new(C[0]);
    for i in 1..N{
        ans +=( ModInt::new(i) * ModInt::new(2).pow(i-1) + ModInt::new(2).pow(i))*ModInt::new(2).pow(N-i-1)*ModInt::new(C[i]);
    }
    ans *= ModInt::new(2).pow(N);
    println!("{}", ans)
