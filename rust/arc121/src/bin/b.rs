...
fn main() {
    ...
    let mut G: Vec<i128> = Vec::new();
    let mut B: Vec<i128> = Vec::new();
    for (a, c) in AC{
        if c.to_string() == "R".to_string(){
            R.push(a);
        }else if c.to_string() == "G".to_string(){
            G.push(a);
        }else{
            B.push(a);
        }
    ...
    let ans = {
        if R.len()%2 == 0{
            odd_ans(&mut G, &mut B, &mut R)
        }else if G.len()%2 == 0{
            odd_ans(&mut R, &mut B, &mut G)
        }else{
            odd_ans(&mut R, &mut G, &mut B)
        }
    };
    println!("{}", ans);
}

fn odd_ans(odd0: &mut Vec<i128>, odd1: &mut Vec<i128>, even: &mut Vec<i128>) -> i128{
    let mut ret = std::i128::MAX;
    odd0.sort();
    odd1.sort();
    // odd0 & odd1
    for i in 0..odd0.len(){
        let a = odd0[i];
        let d = nearest_distance(&odd1, a);
        ret = min(ret, d);
    }
    if even.len() == 0{
        return ret
    ...
    // odd0 + odd1 & even
    let mut odd0even = std::i128::MAX;
    let mut odd1even = std::i128::MAX;
    for i in 0..even.len(){
        let a = even[i];
        odd0even = min(odd0even, nearest_distance(&odd0, a));
        odd1even = min(odd1even, nearest_distance(&odd1, a));
    }
    ret = min(ret, odd0even + odd1even);
    ret
}
...
// fn lower_bound(A: &Vec<i128>, a: i128)-> usize {
//     let N = A.len();
//     if A[N-1] < a{return N}
//     let mut l = 0;
//     let mut r = N;
