...
fn main() {
    ...
    let mut X: Vec<(i64, usize)> = Vec::new();
    let mut Y: Vec<(i64, usize)> = Vec::new();
    for i in 0..N{
        let (x,y) = XY[i];
        X.push((x, i));
        Y.push((y,i));
    ...
    X.sort();
    Y.sort();
    let Mx = (X[0].0-X[N-1].0).abs();
    let My = (Y[0].0-Y[N-1].0).abs();
    let M = max(Mx, My);
    if (Mx == M) && (My == M){
        if ((X[0].1 == Y[0].1) || (X[0].1 == Y[N-1].1)) && ((X[N-1].1 == Y[0].1)||(X[N-1].1 == Y[N-1].1)){
            let mut Z = Vec::new();
            // Z.append(Mx);
            // Z.append(My);
            Z.push((X[0].0 - X[N-2].0).abs());
            Z.push((X[1].0 - X[N-1].0).abs());
            Z.push((Y[0].0 - Y[N-2].0).abs());
            Z.push((Y[1].0 - Y[N-1].0).abs());
            println!("{}", Z.iter().max().unwrap());
            return
        }
    }
    let mut Z = Vec::new();
...
#[warn(unused_imports)]
use crate::dynamic_modint::primitive_root;
