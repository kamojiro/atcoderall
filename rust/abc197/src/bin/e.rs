...
fn main() {
    ...
    let mut LR = vec![(std::i128::MAX, std::i128::MIN); N];
    for &(x, c) in &XC{
        LR[c].0 = LR[c].0.min(x);
        LR[c].1 = LR[c].1.max(x);
    }
    ...
    for i in 0..(n-1){
        let (bl, br) = lr[i];
        let (l, r) = lr[i+1];
        dp[i+1][0] = (dp[i][0]+(r-bl).abs()+(r-l)).min(dp[i][1] + (r-br).abs()+(r-l));
        dp[i+1][1] = (dp[i][0]+(l-bl).abs()+(r-l)).min(dp[i][1] + (l-br).abs()+(r-l));
    }
    // eprintln!("{:?} {}", lr, n);
    // eprintln!("{:?} {}", dp, n);
    let ans = (dp[n-1][0] + lr[n-1].0.abs()).min(dp[n-1][1] + lr[n-1].1.abs());
    println!("{}", ans);
}
