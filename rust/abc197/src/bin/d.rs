...
fn main() {
    ...
    let x = ((xl+x0)/2.0, (yl+y0)/2.0);
    // eprintln!("{:?}", x);
    let xp = (x0 - x.0, y0 - x.1);
    // eprintln!("{:?}", xp);
    let theta = 2.0 * std::f64::consts::PI / (N as f64);
    let rotated = (theta.cos()*xp.0 - theta.sin()*xp.1, theta.sin()*xp.0 + theta.cos()*xp.1);
    // eprintln!("{:?}", rotated);
    println!("{} {}", x.0+rotated.0, x.1+rotated.1);
}
