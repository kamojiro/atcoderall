...
fn main() {
    ...
    let mut cost = vec![std::usize::MAX; N*N];
    cost[0*N+(N-1)] =0;
    let mut queue = VecDeque::new();
    queue.push_back(0*N+(N-1));
    ...
    for v in 0..N{
        let x = v*N + v;
        if cost[x] < std::usize::MAX{
            ans = ans.min(cost[x]*2)
        }
    }
    for &(a,b,_) in &ABC{
        let x = a*N+b;
        if cost[x] < std::usize::MAX{
            ans = ans.min(cost[x]*2+1);
        }
        let y = b*N+a;
        if cost[y] < std::usize::MAX{
            ans = ans.min(cost[y]*2+1);
        }
    }
