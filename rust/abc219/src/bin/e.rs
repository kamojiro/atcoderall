...
fn main() {
    ...
    'hori: for p in 1..(1 << (N*N)){
        ...
        for i in 0..N{
            for j in 0..N{
                if A[i][j] == 1{
                    if !area.contains(&(i*N+j)){
                        continue 'hori;
                    }
                }
            }
        }
        if check_connectness(N, &area) && check_holeless(N, &area){
            ans += 1;
        }
    }
    println!("{}", ans);
...
fn check_connectness(n: usize, area: &Vec<usize>) -> bool{
    let mut board = vec![vec![0;n];n];
    for &a in area{
        board[a/n][a%n] = 1;
    }
    let mut tree = UnionFind::new(n*n);
    for i in 0..n{
        for j in 0..(n-1){
            if board[i][j] == 1 && board[i][j+1] == 1{
                tree.union(i*n+j, i*n+j+1);
            }
        }
    }
    for j in 0..n{
        for i in 0..(n-1){
            if board[i][j] == 1 && board[i+1][j] == 1{
                tree.union(i*n+j, (i+1)*n+j);
            }
        }
    }
    for i in 0..area.len(){
        if !tree.equiv(area[0], area[i]){
            return false
        }
    }
    true
...
fn check_holeless(n: usize, area: &Vec<usize>) -> bool{
    let mut board = vec![vec![false;n];n];
    for &a in area{
        board[a/n][a%n] = true;
    }
    let mut tree = UnionFind::new(n*n+1);
    // eprintln!("{:?}", area);
    for i in 0..n{
        for j in 0..n{
            ...
            if 1 <= i && i < n-1 && 1 <= j && j < n-1{
                if !board[i-1][j]{
                    tree.union((i-1)*n+j, i*n+j);
                }
                if !board[i+1][j]{
                    tree.union((i+1)*n+j, i*n+j);
                }
                if !board[i][j+1]{
                    tree.union(i*n+j+1, i*n+j);
                }
                if !board[i][j-1]{
                    tree.union(i*n+j-1, i*n+j);
                }
                ...
                //     for jj in (j-1)..(j+2){
                //         if !board[ii][jj]{
                //             tree.union(i*n+j, ii*n+jj);
                //         }
                //     }
                // }
            }else{
                tree.union(i*n+j, n*n);
            }
        }
    }
    // eprintln!("aaa {:?}", area);
