...
fn main() {
    ...
    for n in 0..N{
        ...
        for i in 0..=X{
            for j in 0..=Y{
                if a <= i && b <= j{
                    if dp[n][i-a][j-b] == std::usize::MAX{
                    }else{
                        dp[n+1][i][j] = dp[n][i][j].min(dp[n][i-a][j-b]+1);
                    }
                } else if a > i && b <= j{
                    if dp[n][0][j-b] == std::usize::MAX{
                    }else{
                        dp[n+1][i][j] = dp[n][i][j].min(dp[n][0][j-b]+1);
                    }
                } else if a <= i && b > j{
                    if dp[n][i-a][0] == std::usize::MAX{
                    }else{
                        dp[n+1][i][j] = dp[n][i][j].min(dp[n][i-a][0]+1);
                    }
                }else{
                    if dp[n][0][0] == std::usize::MAX{
                    }else{
                        dp[n+1][i][j] = dp[n][i][j].min(dp[n][0][0]+1);
                    }
                }
            }
        }
