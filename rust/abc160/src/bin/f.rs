...
fn main() {
    ...
    let comblist = CombinatoricsTable::<Mod10>::new(1, N+1);
    dfs1(0, 0, &edges, &mut size, &mut dp, &comblist);
    let mut ans = vec![ModInt::<_, Mod10>::new(1); N];
    dfs2(0, 0, &edges, &mut ans, ModInt::new(1), &size, &dp, &comblist);
    for &a in &ans{
        println!("{}", a);
    }
}

fn dfs1(s: usize, before: usize, edges: &Vec<Vec<usize>>, size: &mut Vec<usize>, dp: &mut Vec<ModInt<usize,Mod10>>, comblist: &CombinatoricsTable<Mod10>){
    for &v in &edges[s]{
        if v == before{continue;}
        dfs1(v, s, edges, size, dp, comblist);
        size[s] += size[v];
        dp[s] = dp[s]*dp[v]*comblist.tw_inj_srcsym(size[v], size[s]);
    }
    size[s] += 1;
}

fn dfs2(s: usize, before: usize, edges: &Vec<Vec<usize>>, ans: &mut Vec<ModInt<usize, Mod10>>, u: ModInt<usize, Mod10>, size: &Vec<usize>, dp: &Vec<ModInt<usize,Mod10>>, comblist: &CombinatoricsTable<Mod10>){
    let n = edges.len();
    ans[s] = dp[s]*u*comblist.tw_inj_srcsym(n-size[s], n-1);
    for &v in &edges[s]{
        if v == before{continue;}
        let z = dp[v]*comblist.tw_inj_srcsym(size[v], n-1);
        let u1 = ans[s]*z.inverse();
        dfs2(v, s, edges, ans, u1, size, dp, comblist);
    }
}
