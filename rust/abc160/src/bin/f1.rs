...
fn main() {
    ...
    let mut dp = vec![ModInt::<_, Mod10>::new(1); N];
    let mut size = vec![0; N];
    let factorial = get_factorial(N);
    let inversed_factorial = get_inversed_factorial(N);
    dfs1(0, 0, &edges, &mut size, &mut dp, &factorial, &inversed_factorial);
    eprintln!("{:?}", dp);
    let mut ans = vec![ModInt::<_, Mod10>::new(1); N];
    dfs2(0, 0, &edges, &mut ans, ModInt::new(1), &size, &dp, &factorial, &inversed_factorial);
    for &a in &ans{
        println!("{}", a);
    }
}

fn dfs1(s: usize, before: usize, edges: &Vec<Vec<usize>>, size: &mut Vec<usize>, dp: &mut Vec<ModInt<usize,Mod10>>, factorial: &Vec<ModInt<usize,Mod10>>, inversed_factorial: &Vec<ModInt<usize,Mod10>>){
    for &v in &edges[s]{
        if v == before{continue;}
        dfs1(v, s, edges, size, dp, factorial, inversed_factorial);
        size[s] += size[v];
        eprintln!("{} {} {}",size[s], size[v], factorial[size[s]]*inversed_factorial[size[v]]*inversed_factorial[size[s]-size[v]]);
        dp[s] = dp[s]*dp[v]*(factorial[size[s]]*inversed_factorial[size[v]]*inversed_factorial[size[s]-size[v]]);
    }
    size[s] += 1;
}

fn dfs2(s: usize, before: usize, edges: &Vec<Vec<usize>>, ans: &mut Vec<ModInt<usize, Mod10>>, u: ModInt<usize, Mod10>, size: &Vec<usize>, dp: &Vec<ModInt<usize,Mod10>>, factorial: &Vec<ModInt<usize,Mod10>>, inversed_factorial: &Vec<ModInt<usize,Mod10>>){
    let n = edges.len();
    ans[s] = dp[s]*u*(factorial[n-1]*inversed_factorial[n-size[s]]*inversed_factorial[size[s]-1]);
    for &v in &edges[s]{
        if v == before{continue;}
        let z = dp[v]*(factorial[n-1]*inversed_factorial[size[v]]*inversed_factorial[n-1-size[v]]);
        let u1 = ans[s]*z.inverse();
        dfs2(v, s, edges, ans, u1, size, dp, factorial, inversed_factorial);
    }
}
...
fn get_factorial(n: usize) -> Vec<static_prime_modint::ModInt<usize, static_prime_modint::Mod10>>{
    let mut ret = vec![static_prime_modint::ModInt::<_, Mod10>::new(1);1];
    for i in 1..=n{
        ret.push(*ret.last().unwrap() * ModInt::new(i));
    }
    ret
}

fn get_inversed_factorial(n: usize) -> Vec<static_prime_modint::ModInt<usize, static_prime_modint::Mod10>>{
    let mut ret = vec![static_prime_modint::ModInt::<_, Mod10>::new(1);1];
    let inversed = get_inversed(n);
    for i in 1..=n{
        ret.push(*ret.last().unwrap() * inversed[i]);
    }
    ret
}
 
fn get_inversed(n: usize) -> Vec<static_prime_modint::ModInt<usize, static_prime_modint::Mod10>>{
    let mut ret = vec![static_prime_modint::ModInt::<_, Mod10>::new(1);2];
    let Q: usize = 1_000_000_007;
    for i in 2..=n{
        ret.push((ModInt::new(0)- ModInt::new(Q/i)) * ret[Q%i]);
    }
    ret
}
