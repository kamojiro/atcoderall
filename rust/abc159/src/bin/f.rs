...
fn main() {
    ...
    let mut dp = vec![vec![vec![ModInt::<usize, Mod9>::new(0); 3]; S+1]; N+1];
    dp[0][0][0] = ModInt::new(1);
    for (i, &a) in A.iter().enumerate(){
        for j in 0..=S{
            if a <= j{
                dp[i+1][j][0] = dp[i][j][0];
                dp[i+1][j][1] = dp[i][j][1] + dp[i][j][0] + dp[i][j-a][0] + dp[i][j-a][1];
                dp[i+1][j][2] = dp[i][j][2] + dp[i][j][1] + dp[i][j-a][1] + dp[i][j-a][0];
            }else{
                dp[i+1][j][0] = dp[i][j][0];
