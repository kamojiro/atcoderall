...
fn main() {
    ...
    let mut data: Vec<usize> = (0..N).collect_vec();
    let mut count = 0;
    let mut s = 0.0;
    loop{
        count += 1;
        s += (0..(N-1)).fold(0.0, |p,i| p+d(XY[data[i]], XY[data[i+1]]));
        if !data.next_permutation(){break}
    }
    println!("{}", s/(count as f64));
}

fn d(x: (i64,i64), y: (i64,i64)) -> f64{
    (((x.0 - y.0)*(x.0 - y.0) + (x.1 - y.1)*(x.1 - y.1)) as f64).sqrt()
}
