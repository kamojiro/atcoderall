...
fn main() {
    ...
    H.insert(0,0);
    let mut dp = vec![vec![vec![std::i128::MAX/2; K+1]; N+1]; N+1];
    dp[0][0][0] = 0;
    for i in 0..N{
        for j in 0..=N{
            for k in 0..=K{
                dp[i+1][i+1][k] = dp[i+1][i+1][k].min(dp[i][j][k] + max(0, H[i+1] - H[j]));
                if k < K{
                    dp[i+1][j][k+1] = dp[i+1][j][k+1].min(dp[i][j][k]);
                }
            }
        }
    }
    println!("{}", (0..(N+1)).map(|x| dp[N][x].iter().min().unwrap()).min().unwrap());
}
