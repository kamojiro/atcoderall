...
fn main() {
    ...
    let mut divs: Vec<usize> = divisors((P-1) as u64).iter().map(|&x| x as usize).collect();
    divs.sort_by(|a, b| b.cmp(a));
    let mut ans = ModInt::<_, Mod9>::new(1);
    let mut count = HashMap::new();
    for i in 0..divs.len(){
        let g = divs[i];
        let mut g_order = (P-1)/g;
        for &d in &divs{
            if (g < d) && (d%g == 0){
                g_order -= count.get(&d).unwrap();
            }
        }
        count.insert(g, g_order);
        ans = ans + ModInt::new((P-1)/g)*ModInt::new(g_order);
    }
    println!("{}", ans);
}
