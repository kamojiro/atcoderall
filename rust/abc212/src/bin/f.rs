...
fn main() {
    input!{
        ...
        M: usize,
        Q: usize,
        mut ABST: [(Usize1,Usize1,usize,usize); M],
        mut XYZ: [(usize,Usize1,usize); Q],
    }
    let mut starts = vec![vec![];N];
    for (i, &(a,_,s,_)) in ABST.iter().enumerate(){
        starts[a].push((s,i));
    }
    starts.iter_mut().for_each(|x| x.sort());
    // let logm: usize = 17; //2^17=131072
    let logm = M.next_power_of_two().trailing_zeros() as usize;
    let mut transfer: Vec<Vec<usize>> = vec![vec![0; M]; logm+1];
    for (j, &(_,t,_,b)) in ABST.iter().enumerate(){
        let lb = starts[t].lower_bound_by_key(&b, |x| x.0);
        transfer[0][j] = if lb < starts[t].len(){
            starts[t][lb].1
        }else{
            j
        };
    ...
    for i in 1..=logm{
        for j in 0..M{
            transfer[i][j] = transfer[i-1][transfer[i-1][j]]
        }
    }
    for &(start_time, start_pos, end_time) in &XYZ{
        let lb = starts[start_pos].lower_bound_by_key(&start_time, |&(t,_)| t);
        if lb == starts[start_pos].len(){
            println!("{}", start_pos+1);
            continue;
        }
        let (_, i) = starts[start_pos][lb];
        if end_time <= ABST[i].2{
            println!("{}", start_pos+1);
            continue;
        }
        let mut now = i;
        ...
        while right - left > 1{
            ...
            while m2 > 0{
                if m2%2 == 1{
                    now2 = transfer[idx][now2];
                }
                m2 /= 2;
                idx += 1;
            }
            if ABST[now2].2 < end_time{
                left = m
            }else{
                right = m
            }
        ...
        while m2 > 0{
            if m2%2 == 1{
                now = transfer[idx][now];
            }
            m2 /= 2;
            idx += 1;
        }
        if ABST[now].3 >= end_time{
            println!("{} {}", ABST[now].0+1, ABST[now].1+1)
        }else{
            println!("{}", ABST[now].1+1)
        }
