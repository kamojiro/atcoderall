...
fn main() {
    ...
    //     accB.push(now);
    // }
    let mut ans = 1000_000_000_0;
    for &a in &A{
        // let x = lower_bound(&B, a) as i64;
        // ans += x*a - accB[x as usize] + (accB[M] - accB[x as usize]) - ((M as i64)-x)*x;
        let x = lower_bound(&B, a).min(M-1);
        let mut p = (a - B[x]).abs();
        if x > 0{
            p = p.min((a - B[x-1]).abs())
        }
...
fn lower_bound(A: &Vec<i64>, a: i64) -> usize {
    // a 以下の個数
    let N = A.len();
    if A[N-1] < a{return N}
    if a < A[0]{return 0}
