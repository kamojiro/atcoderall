...
fn main() {
    ...
    for &c in &C{
        color.entry(c).or_insert(0);
    }
    let mut visited = vec![false; N];
    visited[0] = true;
    let mut ans = Vec::new();
    dfs(&C, &edges, &mut visited, &mut color, 0, &mut ans);
    ans.sort();
    for &a in &ans{
        println!("{}", a+1);
    }
...
fn dfs(C: &Vec<usize>, edges: &Vec<Vec<usize>>, visited: &mut Vec<bool>, color: &mut HashMap<usize, usize>,v: usize, ans: &mut Vec<usize>){
    if color[&C[v]] == 0{
        ans.push(v);
    }
    let counter = color.entry(C[v]).or_insert(0);
    *counter += 1;
    for &w in &edges[v]{
        if visited[w]{continue;}
        visited[w] = true;
        dfs(C, edges, visited, color, w, ans);
        visited[w] = false;
    }
    let counter = color.entry(C[v]).or_insert(0);
    *counter -= 1;
