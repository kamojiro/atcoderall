...
fn main() {
    ...
    let mut corresponding = HashMap::new();
    let mut count: usize = 0;
    for i in 0..3{
        for &s in &S[i]{
            if !corresponding.contains_key(&s){
                corresponding.insert(s, count);
                count += 1;
            }
        }
    }
    ...
    for i in 0..3{
        for &s in &S[i]{
            T[i].push(*corresponding.get(&s).unwrap());
        }
    }
    
    'permutation: for perm in (0..10).permutations(count){
        // eprintln!("{:?} {:?}", perm, T);
        for i in 0..3{
            if perm[T[i][0]] == 0{
