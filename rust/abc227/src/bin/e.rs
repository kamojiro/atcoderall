...
fn main() {
    ...
    //         }
    //     }
    //     let mut ans1 = ModInt::<_, Mod10>::new(1);
    //     let mut ans2 = ModInt::<_, Mod10>::new(1);
    //     let n = count[0] + count[1] + count[2];
    //     let k = count[0];
    //     for i in (n-k+1)..=n{
    //         ans1 = ans1 * ModInt::new(i);
    //     }
    //     for i in 1..=k{
    //         ans1 = ans1 * ModInt::new(i).inverse();
    //     }
    //     let n = count[1] + count[2];
    //     let k = count[1];
    //     for i in (n-k+1)..=n{
    //         ans2 = ans2 * ModInt::new(i);
    //     }
    //     for i in 1..=k{
    //         ans2 = ans2 * ModInt::new(i).inverse();
    //     }
    //     let mut ans = ans1.value() as u128;
    //     ans *= ans2.value() as u128;
    //     println!("{}", ans);
    //     return;
    // }
    let mut p = 0;
    ...
    set.insert(S.clone());
    dfs(S.len(), 0, K, p, &mut set);
    // eprintln!("{:?}", set);
    println!("{}", set.len())
}

fn dfs(n: usize, depth: usize, max_depth: usize, p: u128, set: &mut HashSet<u128){
    if depth == max_depth{return}
    for i in 0..(n-1){
        let swap = p%(1<<((i+1)*2))/(1<<(i*2));
        p -= 
        if !set.contains(p){
            set.insert(p);
            dfs(n, depth+1, max_depth, p, set)
        }
        let swap = s[i];
        s[i] = s[i+1];
        s[i+1] = swap;
