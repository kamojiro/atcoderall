...
fn main() {
    ...
    let (prev, count) = count_vertex_of_subtree(&edges, s);
    let mut ans = ModInt::<_, Mod10>::new(0);
    for i in 0..N {
        if edges[i].len() == 1 {
            continue;
        }
        ans += ModInt::new(2).pow(N - 1);
        for &w in &edges[i] {
            if w == prev[i] {
                continue;
            }
            ans -= ModInt::new(2).pow(count[w]) - 1;
        }
        ans -= ModInt::new(2).pow(N - count[i]) - 1;
        ans -= 1;
    }
    let q = ModInt::new(2).pow(1_000_000_005).pow(N);
    println!("{}", ans * q);
}

fn count_vertex_of_subtree(edges: &Vec<Vec<usize>>, root: usize) -> (Vec<usize>, Vec<usize>) {
    let n = edges.len();
