...
fn main() {
    ...
    // eprintln!("{:?}", squares);
    // while a*a < N{
    //     let x = squares.upper_bound(&(a*a+N));
    //     eprintln!("aan{} a{} x{}",a*a+N, a, x);
    //     ans += ModInt::new(x-a-1);
    //     a += 1;
    // }
    for i in 1..n{
        ans += ModInt::new(i);
    ...
    let mut now = N;
    for i in 1..=n{
        let mut x = if (i*i+N)%(2*i) == 0{(i*i+N)/(2*i)+1}else{(i*i+N+2*i-1)/(2*i)};
        // eprintln!("i {} x {}", i, x);
        x = x.max(n-1);
        if x < N{
            ans += ModInt::new(i-1)*ModInt::new(now-x);
            now = x;
        }
