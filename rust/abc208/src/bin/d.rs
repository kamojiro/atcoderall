...
fn main() {
    ...
    for (i, &(a,b,c)) in ABC.iter().enumerate(){
        edges[a-1].push((b-1,c,i+1));
    }
    let mut ans = 0;
    for s in 0..N{
        let cost = dijkstra(&edges, s);
        for t in 0..N{
            let mut now = cost[0][t];
            for n in 1..=N{
                now = now.min(cost[n][t]);
...
fn dijkstra(edges: &Vec<Vec<(usize, usize, usize)>>, start: usize) -> Vec<Vec<usize>>{
    let mut queue = BinaryHeap::new();
    let N = edges.len();
    let mut ret = vec![vec![std::usize::MAX; N]; N+1];
    queue.push((Reverse(0), 0, start));
    while let Some((Reverse(time), k, city)) = queue.pop(){
        if ret[k][city] < std::usize::MAX{
            continue;
        }
        ret[k][city] = time;
        for &(next, cost, nextk) in &edges[city]{
            if ret[nextk][next] < std::usize::MAX{continue;}
            if k < nextk{
                queue.push((Reverse(time+cost), nextk, next))
            }
            if ret[k][next] 
            queue.push((Reverse(time+cost), k, next))
        }
    }
    ret
}
