...
fn main() {
    ...
    for (i, &x) in xx.iter().enumerate(){
        coordinate.insert(x, i);
    }
    let mut length = vec![0; xx.len()];
    let mut tree = UnionFind::new(xx.len());
    let mut cut = vec![false; xx.len()];
    for &(c,x) in &CX{
        if c == 1{
            let z = *coordinate.get(&x).unwrap();
            cut[z] = true;
        }
    }
    for i in 0..(xx.len()-1){
        length[i] = xx[i+1] - xx[i];
    }
    for i in 1..(xx.len()-1){
        if !cut[i]{
            let l = length[tree.find(i)] + length[tree.find(i-1)];
            tree.union(i, i-1);
            length[tree.find(i)] = l;
        }
    }
    let mut ans = Vec::new();
    for &(c,x) in CX.iter().rev(){
        let i = *coordinate.get(&x).unwrap();
        if c == 1{
            let l = length[tree.find(i)] + length[tree.find(i-1)];
            tree.union(i-1,i);
            length[tree.find(i)] = l;
        }else{
            ans.push(length[tree.find(i)])
        }
    }
