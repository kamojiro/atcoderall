...
fn main() {
    ...
    let mut good = vec![vec![ModInt::<_,Mod9>::new(0);N*2];N*2];
    for &(a,b) in &AB{
        good[a][b] = ModInt::new(1);
        good[b][a] = ModInt::new(1);
    }
    let factorial = get_factorial(N);
    let inversed_factorial = get_inversed_factorial(N);
    
    let mut dp = vec![vec![ModInt::<_,Mod9>::new(0);N+1]; N*2+1];
    for j in 0..=N*2{
        dp[j][0] = ModInt::new(1);
    }
    for i in 1..(N+1){
        // factorial *= ModInt::new(i);
        for j in 0..(N*2+1-i*2){
            for k in 1..=i{
                dp[j][i] = dp[j][i] + dp[j+1][k-1]*dp[j+k*2][i-k]*comb(&factorial, &inversed_factorial, i, k)*good[j][j+k*2-1];
            }
        }
    }
    println!("{}", dp[0][N])
...
fn comb(f: &Vec<ModInt<usize,Mod9>>, i: &Vec<ModInt<usize,Mod9>>, n: usize, k: usize) -> ModInt<usize,Mod9>{
    f[n]*i[k]*i[n-k]
}
