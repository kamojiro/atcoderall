...
fn main() {
    ...
    visited[0] = true;
    dfs(&edges, 0, &mut visited, &mut path);
    let mut dp = vec![vec![std::usize::MAX; 1]; N+1];
    ...
    for &v in &path{
        ...
        visited[v] = true;
        let a = A[v];
        let x = dp.lower_bound_by(|x| x.last().unwrap().cmp(&a));
        dp[x].push(a);
        ans[v] = dp.lower_bound_by(|x| x.last().unwrap().cmp(&(std::usize::MAX-1)));
        stack.push(x);
    }
    for &a in &ans{
        println!("{}", a)
...
fn dfs(edges: &Vec<Vec<usize>>, v: usize, visited: &mut Vec<bool>, path: &mut Vec<usize>){
    path.push(v);
    for &w in &edges[v]{
        if visited[w]{continue;}
        visited[w] = true;
        dfs(edges, w, visited, path);
        path.push(v);
    }
}
