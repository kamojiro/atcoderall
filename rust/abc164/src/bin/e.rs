...
fn main() {
    ...
    let P = 50*(N-1);
    S = S.min(P);
    let mut visited = vec![vec![std::usize::MAX; P+1]; N];
    let mut ans = vec![std::usize::MAX; N];
    ans[0] = 0;
    let mut queue = BinaryHeap::new();
    queue.push((Reverse(0), 0, S));
    while let Some((Reverse(time), city, money)) = queue.pop(){
        if ans[city] == std::usize::MAX{
            ans[city] = time;
        }
        if visited[city][money] < std::usize::MAX{continue;}
        visited[city][money] = time;
        let (c,d) = CD[city];
        if money + c <= P{
            queue.push((Reverse(time + d), city, money + c))            
        }
        for &(next_city, a, b) in &edges[city]{
            if money < a{continue;}
            if visited[next_city][money-a] < std::usize::MAX{continue;}
            queue.push((Reverse(time+b), next_city, money-a));
        }
    }
    for i in 1..N{
        println!("{}", ans[i])
