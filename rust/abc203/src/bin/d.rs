...
fn main() {
    ...
    while r - l > 1{
        let m = (l+r)/2;
        // eprintln!("{} {}",m ,is_median(N, K, &A, m));
        if is_median(N, K, &A, m){
            r = m
        }else{
            l = m
        }
...
fn is_median(N: usize, K : usize, A: &Vec<Vec<i64>>, a: i64) -> bool{
    ...
    for i in 0..(N-K+1){
        for j in 0..(N-K+1){
            let t = B[i+K][j+K] - B[i][j+K] - B[i+K][j] + B[i][j];
            // eprintln!("{} {} {} {} {}", i, j,i+K,j+K ,t);
            if t >= k{
                return true;
            }
        }
...
#[warn(dead_code)]
fn is_old_median(N: usize, K : usize, A: &Vec<Vec<i64>>, a: i64) -> bool{
    let mut F = vec![vec![vec![0,0]; N-K+1];N-K+1];
    // <a, =a, a<
    for i in 0..K{
        for j in 0..K{
            if A[i][j] <= a{
