...
fn main() {
    ...
    xmy.sort_by_key(|x| x.1);
    xpy.sort_by_key(|x| x.1);
    let mut m = 0;
    let mut mxxmy = Vec::new();
    for &(x, _) in &xmy{
        m = m.max(x);
        mxxmy.push(m);
    }
    mxxmy.push(m);
    let mut m = 0;
    let mut myxmy = Vec::new();
    for &(x, _) in xmy.iter().rev(){
        m = m.max(x);
        myxmy.push(m);
    }
    myxmy.reverse();
    let mut m = 0;
    let mut mxxpy = Vec::new();
    for &(x, _) in &xpy{
        m = m.max(x);
        mxxpy.push(m);
    }
    mxxmy.push(m);
    let mut m = 0;
    let mut myxpy = Vec::new();
    for &(x, _) in xpy.iter().rev(){
        m = m.max(x);
        myxpy.push(m);
    }
    myxpy.reverse();
    ...
    for i in 0..N{
        let (x,y) = XY[i];
        let a = x-y;
        let idx = xmy.upper_bound_by_key(&a, |x| x.1);
        ans = ans.max(mxxmy[idx.min(N-1)]-x);
        let idx = xmy.lower_bound_by_key(&a, |x| x.1);
        ans = ans.max(myxmy[idx.min(N-1)]-y);
        let a = x+y;
        let idx = xpy.upper_bound_by_key(&a, |x| x.1);
        ans = ans.max(mxxpy[idx.min(N-1)]-x);
        let idx = xpy.lower_bound_by_key(&a, |x| x.1);
        ans = ans.max(myxpy[idx.min(N-1)]-y);        
    }
    println!("{}", ans)
