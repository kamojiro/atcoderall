...
fn main() {
    ...
    let mut ans = ModInt::<_, Mod10>::new(2).pow(n) - ModInt::new(1);
    ans -= combination(n, a);
    ans -= combination(n, b);
    println!("{}", ans);
}

fn combination(n: usize, a: usize) -> ModInt<usize,Mod10>{
    let mut ret = ModInt::<_, Mod10>::new(1);
    let b = if a < n-a{a}else{n-a};
    for i in 0..b{
        ret *= ModInt::new(n-i);
        ret *= ModInt::new(i+1).inverse();
    }
    ret
}
