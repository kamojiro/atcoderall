...
fn main() {
    ...
    let mut dp = vec![vec![std::i128::MIN/2; 3]; N];
    dp[0][1] = 0;
    dp[0][2] = A[0];
    dp[0][0] = 0;
    dp[1][1] = A[0].max(A[1]);
    for i in 2..N{
        let a = A[i];
        if i%2 == 0{
            dp[i][0] = dp[i-1][0].max(dp[i-2][0]+a);
            dp[i][1] = dp[i-1][1].max(dp[i-2][1]+a);
            dp[i][2] = dp[i-2][2]+a;
        }else{
            dp[i][0] = dp[i-1][1].max(dp[i-2][0]+a);
            dp[i][1] = dp[i-1][2].max(dp[i-2][1]+a);
        }
    }
    println!("{}", dp[N-1][1]);
