...
fn main() {
    ...
    (0..N).for_each(|x| A[x] &= 2.pow(M as u32) - 1 - s1);
    let mut basis = Vec::new();
    let mut used = vec![false; N];
    for i in (0..M).rev() {
        // i bit目がある vector を探す
        match (0..N).find(|&x| (A[x] >> i & 1 == 1) && !used[x]) {
            Some(x) => {
                used[x] = true;
                let v = A[x];
                // 取得済みの基底から i bit目を消す
                (0..basis.len()).for_each(|y| {
                    if basis[y] >> i & 1 == 1 {
                        basis[y] ^= v
                    }
                });
                basis.push(v);
                // 未取得の vector に基底を適用して、i bit目を消す
                (0..N).for_each(|y| {
                    if A[y] >> i & 1 == 1 {
                        A[y] ^= v
                    }
    ...
    let s2 = basis.iter().fold(0, |s, x| s ^ x);
    println!("{}", s1 + s2 * 2);
}
