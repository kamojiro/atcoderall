...
fn main() {
    ...
    let mut color = vec![M; N];
    // let mut set = HashSet::new();
    let mut index = vec![0; M];
    for i in 0..M{
        let a = A[i][0];
        if color[a] < M{
            candidate.push_back((color[a], i));
        }else{
            color[a] = i;
        }
    }
    while let Some((x,y)) = candidate.pop_front(){
        // eprintln!("c {} x{} y{}",A[x][index[x]], x,y);
        ...
        if index[x] < A[x].len(){
            ...
            // eprintln!("{}", ax);
            if color[ax] < M{
                // eprintln!("add candi {} {} {}", ax, x, color[ax]);
                candidate.push_back((x, color[ax]));
            }else{
                color[ax] = x;
            }
        }
        ...
        if index[y] < A[y].len(){
            let ay = A[y][index[y]];
            if color[ay] < M{
                // eprintln!("add candi {} {} {}", ay, y, color[y]);
                candidate.push_back((y, color[ay]));
            }else{
                color[ay] = y;
            }
        }
