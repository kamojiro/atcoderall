...
fn main() {
    ...
    AB.sort();
    let mut dp = vec![vec![ModInt::<_, Mod9>::new(0); M+1]; M+1];
    dp[0][0] = ModInt::new(1);


    let mut segs = vec![SegmentTree::new(M+1, || ModInt::<_, Mod9>::new(0), |a,b| a+b); M+1];
    segs[0].update(0, ModInt::new(1));
    for &(a,b) in &AB{
        for j in b..=M{
            dp[a][j] = segs[j-b].query(0..(a+1));
            segs[j].update(a, dp[a][j]);
        }
    }
