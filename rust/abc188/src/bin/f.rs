...
fn main() {
    ...
    println!("{}", solve(Y,X,&mut memo));
    // println!("{:?}", memo);
}

fn solve(y: i128, x: i128, memo: &mut HashMap<i128,i128>) -> i128 {
    return match memo.get(&y){
        Some(&number) => number,
        None => {
            let z: i128 = {
                if y == 1{
                    ...
                    *[solve(y/2,x,memo)+1, (y-x).abs()].iter().min().unwrap()
                }else{
                    *[solve((y+1)/2,x,memo)+2,solve((y-1)/2,x,memo)+2, (y-x).abs()].iter().min().unwrap()
                }
            };
            // println!("{} {}",y, z );
            memo.insert(y, z);
